...
describe('parseInput — keyboard', () => {
  it('parses regular characters', () => {
    const events = parseInput('hello')
    expect(events).toHaveLength(1)
    expect(events[0]).toMatchObject({ kind: 'key', input: 'hello', key: expect.objectContaining({ return: false }) })
  })

  it('parses return key', () => {
    const events = parseInput('\r')
    expect(events).toHaveLength(1)
    expect(events[0]).toMatchObject({ kind: 'key', input: '', key: expect.objectContaining({ return: true }) })
  })

  it('parses backspace', () => {
    const events = parseInput('\x7f')
    expect(events).toHaveLength(1)
    expect(events[0]).toMatchObject({ kind: 'key', input: '', key: expect.objectContaining({ backspace: true }) })
  })
  ...
  it('treats newlines inside bracketed paste as literal characters', () => {
    ...
    expect(events).toHaveLength(1)
    expect(events[0]).toMatchObject({ kind: 'key', input: 'hello\nworld', key: expect.objectContaining({ return: false }) })
  })
  ...
  it('treats \\r inside bracketed paste as \\n', () => {
    ...
    const events = parseInput(paste)
    expect(events).toHaveLength(1)
    expect(events[0]).toMatchObject({ kind: 'key', input: 'line1\nline2', key: expect.objectContaining({ return: false }) })
  })
  ...
  it('handles text before and after bracketed paste', () => {
    ...
    expect(events[1]).toMatchObject({ kind: 'key', input: 'foo\nbar' })
    expect(events[2]).toMatchObject({ kind: 'key', input: '!' })
  })
  ...
  it('handles single-line bracketed paste without newlines', () => {
    ...
    const events = parseInput(paste)
    expect(events).toHaveLength(1)
    expect(events[0]).toMatchObject({ kind: 'key', input: 'just text' })
  })
})
